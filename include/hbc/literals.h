...
HBC_API Result hbc_literals_format_for_opcode(HBC *hbc, u8 opcode, u32 arg3, u32 arg4, u32 arg5, char **out);
...
HBC_API Result hbc_literals_get(HBC *hbc, HBCLiteralKind kind, u32 num_items, u32 primary_id, u32 secondary_id, const char **out_formatted);
...
HBC_API Result hbc_literals_register(HBC *hbc, HBCLiteralKind kind, u32 num_items, u32 primary_id, u32 secondary_id, u32 from_addr);
...
HBC_API Result hbc_literals_scan_pool(HBC *hbc, HBCLiteralKind kind, HBCPoolGroup **out, u32 *out_count);

/* Enumerate the current cache contents. The returned array is owned by the
 * HBC and valid until the next cache mutation. */
HBC_API Result hbc_literals_list(HBC *hbc, const HBCLiteralEntry **out, u32 *out_count);

/* Drop every cached entry and every xref. Fast. Does not affect the SLP
 * pools themselves. */
HBC_API void hbc_literals_reset(HBC *hbc);

/* Number of cached entries. 0 before any register / scan call. */
HBC_API u32 hbc_literals_count(HBC *hbc);

/* Absolute file offset of the named pool, or 0 if the pool is empty/absent. */
HBC_API u32 hbc_get_pool_paddr(HBC *hbc, HBCLiteralKind kind);
HBC_API u32 hbc_get_pool_size(HBC *hbc, HBCLiteralKind kind);

/* Value pool paddr/size for objects (arrays & object-keys reuse the kind-based
 * accessors; object values is a distinct pool). */
HBC_API u32 hbc_get_object_values_paddr(HBC *hbc);
HBC_API u32 hbc_get_object_values_size(HBC *hbc);
...
HBC_API void hbc_set_inline_literals(bool on);
HBC_API bool hbc_get_inline_literals(void);
