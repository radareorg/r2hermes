...
#define RETURN_IF_ERROR(expr) do { \
    Result result = (expr); \
    if (result.code != RESULT_SUCCESS) return result; \
} while (0)

#define MALLOC_OR_ERROR(ptr, type, count) do { \
    (ptr) = (type*)malloc(sizeof(type) * (count)); \
    if (!(ptr)) { \
        Result result = {RESULT_ERROR_MEMORY_ALLOCATION, "Memory allocation failed"}; \
        return result; \
    } \
} while (0)
