...
$(LIB_RELOC_OBJ): $(LIB_OBJ)
	$(CC) $(LD_RELOC_FLAGS) -o $@ $^

$(STATIC_LIB): $(LIB_RELOC_OBJ)
	$(RM) $@
	ar rcs $@ $^

$(MAIN_RELOC_OBJ): $(MAIN_OBJ) $(LIB_OBJ)
	$(CC) $(LD_RELOC_FLAGS) -o $@ $^
...
test: $(BIN_FILE) $(TEST_BIN)
	...
	$(BIN_FILE) eval test/bins/hbc/bespoke_eval.hbc | grep -q '0x000000b4 f=0 +0x4 func=0x000000b0 strict=false name=global'
