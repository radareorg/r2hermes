...
static Result add(TokenString* ts, Token* t) {
    if (!t) return ERROR_RESULT(RESULT_ERROR_MEMORY_ALLOCATION, "alloc token");
    return token_string_add_token(ts, t);
}

static Token* reg_l(int r) { return create_left_hand_reg_token(r); }
static Token* reg_r(int r) { return create_right_hand_reg_token(r); }

static Token* num_token_u32(u32 v) {
    char buf[32]; snprintf(buf, sizeof(buf), "%u", v); return create_raw_token(buf);
}

static Token* num_token_i32(i32 v) {
    char buf[32]; snprintf(buf, sizeof(buf), "%d", v); return create_raw_token(buf);
}

static Token* double_token(u32 lo, u32 hi) {
    union { u64 u; double d; } u; u.u = ((u64)hi << 32) | (u64)lo;
    char buf[64];
...
static Token* quoted_string(HBCReader* r, u32 sid) {
    if (r && r->strings && sid < r->header.stringCount) {
        ...
        StringBuffer sb; if (string_buffer_init(&sb, n + 8).code != RESULT_SUCCESS) return NULL;
        string_buffer_append(&sb, "\"");
        for (size_t i = 0; i < n; i++) {
            char c = s[i];
            if (c == '\\' || c == '"') { string_buffer_append_char(&sb, '\\'); }
            if ((unsigned char)c < 0x20) {
                char tmp[8]; snprintf(tmp, sizeof(tmp), "\\x%02x", (unsigned char)c); string_buffer_append(&sb, tmp);
            } else {
...
Result translate_instruction_to_tokens(const ParsedInstruction* insn_c, TokenString* out) {
    if (!insn_c || !out || !insn_c->inst) return ERROR_RESULT(RESULT_ERROR_INVALID_ARGUMENT, "translate: bad args");
    ...
    switch (op) {
        ...
        case OP_MovLong: {
            /* dest <- src */
            RETURN_IF_ERROR(add(out, reg_l((int)insn->arg1)));
        ...
        case OP_LoadParamLong: {
            ...
            RETURN_IF_ERROR(add(out, create_assignment_token()));
            /* a<idx> */
            char buf[32]; snprintf(buf, sizeof(buf), "a%u", insn->arg2);
        ...
        case OP_Mod: {
            const char* opstr = (op==OP_Add||op==OP_AddN)?"+":(op==OP_Sub||op==OP_SubN)?"-":(op==OP_Mul||op==OP_MulN)?"*":(op==OP_Div||op==OP_DivN)?"/":"%";
            RETURN_IF_ERROR(add(out, reg_l((int)insn->arg1)));
            RETURN_IF_ERROR(add(out, create_assignment_token()));
            RETURN_IF_ERROR(add(out, reg_r((int)insn->arg2)));
            RETURN_IF_ERROR(add(out, create_raw_token(opstr)));
            RETURN_IF_ERROR(add(out, reg_r((int)insn->arg3)));
            break;
        }
        case OP_Negate: case OP_Not: case OP_BitNot: {
        ...
        case OP_Catch: {
            /* catch(rN) marker */
            RETURN_IF_ERROR(add(out, create_catch_block_start_token((int)insn->arg1)));
            break;
        }
        ...
        case OP_CreateInnerEnvironment: {
            RETURN_IF_ERROR(add(out, reg_l((int)insn->arg1)));
            RETURN_IF_ERROR(add(out, create_assignment_token()));
            StringBuffer sb; string_buffer_init(&sb, 48);
            string_buffer_append(&sb, "new_inner_env(");
            char nb[16]; snprintf(nb, sizeof(nb), "r%u", (unsigned)insn->arg2); string_buffer_append(&sb, nb);
            string_buffer_append(&sb, ", "); snprintf(nb, sizeof(nb), "%u", (unsigned)insn->arg3); string_buffer_append(&sb, nb);
            string_buffer_append(&sb, ")");
            Token* t = create_raw_token(sb.data ? sb.data : "new_inner_env(r0,0)"); string_buffer_free(&sb);
        ...
        case OP_CreateClosureLongIndex: {
            ...
            char nb[16]; snprintf(nb, sizeof(nb), "%u", (unsigned)((op==OP_CreateClosure)? insn->arg3 : insn->arg3)); string_buffer_append(&sb, nb);
            string_buffer_append(&sb, ", r"); snprintf(nb, sizeof(nb), "%u", (unsigned)insn->arg2); string_buffer_append(&sb, nb);
            string_buffer_append(&sb, ")");
            Token* t = create_raw_token(sb.data ? sb.data : "closure(fn_0,r0)"); string_buffer_free(&sb);
            if (!t) return ERROR_RESULT(RESULT_ERROR_MEMORY_ALLOCATION, "oom");
            RETURN_IF_ERROR(add(out, t));
            break;
        }
        ...
        case OP_CreateGeneratorClosureLongIndex: {
            ...
            char nb[16]; snprintf(nb, sizeof(nb), "%u", (unsigned)insn->arg3); string_buffer_append(&sb, nb);
            string_buffer_append(&sb, ", r"); snprintf(nb, sizeof(nb), "%u", (unsigned)insn->arg2); string_buffer_append(&sb, nb);
            string_buffer_append(&sb, ")");
            Token* t = create_raw_token(sb.data ? sb.data : "gen_closure(fn_0,r0)"); string_buffer_free(&sb);
            if (!t) return ERROR_RESULT(RESULT_ERROR_MEMORY_ALLOCATION, "oom");
            RETURN_IF_ERROR(add(out, t));
            break;
        }
        ...
        case OP_CreateGeneratorLongIndex: {
            ...
            string_buffer_append(&sb, ", r"); snprintf(nb, sizeof(nb), "%u", (unsigned)insn->arg2); string_buffer_append(&sb, nb);
            string_buffer_append(&sb, ")");
            Token* t = create_raw_token(sb.data ? sb.data : "generator(fn_0,r0)"); string_buffer_free(&sb);
            if (!t) return ERROR_RESULT(RESULT_ERROR_MEMORY_ALLOCATION, "oom");
            RETURN_IF_ERROR(add(out, t));
            break;
        }
        ...
        case OP_GetByIdLong: {
            ...
            RETURN_IF_ERROR(add(out, reg_r((int)insn->arg2)));
            u32 sid = insn->arg4;
            const char* s = NULL; if (insn->hbc_reader && insn->hbc_reader->strings && sid < insn->hbc_reader->header.stringCount) s = insn->hbc_reader->strings[sid];
            bool ident = true; if (!s || !*s) ident = false; else {
                if (!(isalpha((unsigned char)*s) || *s=='_' || *s=='$')) ident = false;
                for (const char* p = s+1; ident && *p; p++) ident = (isalnum((unsigned char)*p) || *p=='_' || *p=='$');
            }
            if (ident) { RETURN_IF_ERROR(add(out, create_dot_accessor_token())); RETURN_IF_ERROR(add(out, create_raw_token(s))); }
            else { RETURN_IF_ERROR(add(out, create_raw_token("["))); RETURN_IF_ERROR(add(out, quoted_string(insn->hbc_reader, sid))); RETURN_IF_ERROR(add(out, create_raw_token("]"))); }
            break;
        }
        case OP_GetByVal: {
            RETURN_IF_ERROR(add(out, reg_l((int)insn->arg1)));
        ...
        case OP_TryGetByIdLong: {
            ...
            string_buffer_append(&sb, ")");
            Token* t = create_raw_token(sb.data ? sb.data : "try_get(r0)"); string_buffer_free(&sb);
            if (!t) return ERROR_RESULT(RESULT_ERROR_MEMORY_ALLOCATION, "oom");
            RETURN_IF_ERROR(add(out, t));
            break;
        }
        ...
        case OP_PutNewOwnByIdLong: {
            ...
            RETURN_IF_ERROR(add(out, reg_l((int)insn->arg1)));
            u32 sid = (op==OP_PutNewOwnByIdShort)? insn->arg3 : (op==OP_PutNewOwnById)? insn->arg3 : insn->arg4;
        ...
        case OP_PutByVal: {
            /* rObj[rKey] = rVal */
            RETURN_IF_ERROR(add(out, reg_l((int)insn->arg1))); /* use LHS token for consistency */
            RETURN_IF_ERROR(add(out, create_raw_token("[")));
            RETURN_IF_ERROR(add(out, reg_r((int)insn->arg2)));
            RETURN_IF_ERROR(add(out, create_raw_token("]")));
            RETURN_IF_ERROR(add(out, create_assignment_token()));
        ...
        case OP_GetEnvironment: {
            ...
            RETURN_IF_ERROR(add(out, reg_l((int)insn->arg1)));
            RETURN_IF_ERROR(add(out, create_assignment_token()));
            StringBuffer sb; string_buffer_init(&sb, 32);
            string_buffer_append(&sb, "env(");
            char nb[16]; snprintf(nb, sizeof(nb), "%u", (unsigned)insn->arg2);
            string_buffer_append(&sb, nb);
            string_buffer_append(&sb, ")");
            Token* t = create_raw_token(sb.data ? sb.data : "env(0)"); string_buffer_free(&sb);
            if (!t) return ERROR_RESULT(RESULT_ERROR_MEMORY_ALLOCATION, "oom");
            RETURN_IF_ERROR(add(out, t));
            break;
        }
        ...
        case OP_NewObjectWithBufferLong: {
            RETURN_IF_ERROR(add(out, reg_l((int)insn->arg1)));
            RETURN_IF_ERROR(add(out, create_assignment_token()));
            StringBuffer sb; Result r = string_buffer_init(&sb, 128);
        ...
        case OP_Call2: {
            ...
            RETURN_IF_ERROR(add(out, reg_r((int)insn->arg3)));
            RETURN_IF_ERROR(add(out, create_raw_token(", ")));
            RETURN_IF_ERROR(add(out, reg_r((int)insn->arg4)));
            RETURN_IF_ERROR(add(out, create_right_parenthesis_token()));
            break;
        }
        case OP_Call3: {
            ...
            RETURN_IF_ERROR(add(out, reg_r((int)insn->arg4)));
            RETURN_IF_ERROR(add(out, create_raw_token(", ")));
            RETURN_IF_ERROR(add(out, reg_r((int)insn->arg5)));
            RETURN_IF_ERROR(add(out, create_right_parenthesis_token()));
            break;
        }
        case OP_Call4: {
            ...
            RETURN_IF_ERROR(add(out, reg_r((int)insn->arg5)));
            RETURN_IF_ERROR(add(out, create_raw_token(", ")));
            RETURN_IF_ERROR(add(out, reg_r((int)insn->arg6)));
            RETURN_IF_ERROR(add(out, create_right_parenthesis_token()));
            break;
        }
        case OP_Call:
