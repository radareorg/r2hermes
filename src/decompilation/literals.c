...
bool is_js_identifier(const char* s) {
    ...
    for (s++; *s; s++) {
        ...
        if (!(isalnum(d) || d=='_' || d=='$')) return false;
    }
    return true;
}
...
static Result append_quoted(StringBuffer* out, const char* s) {
    ...
    for (const char* p = s; p && *p; p++) {
        ...
        if (c == '"' || c == '\\') {
            ...
            char tmp[8]; snprintf(tmp, sizeof(tmp), "\\x%02x", c);
...
Result format_property_from_string_id(HBCReader* r, u32 string_id, StringBuffer* out) {
    const char* s = NULL;
    if (r && r->strings && string_id < r->header.stringCount) s = r->strings[string_id];
    if (is_js_identifier(s)) {
        return string_buffer_append(out, s ? (const char*)"." : ".");
    }
    RETURN_IF_ERROR(string_buffer_append(out, "["));
    if (s) { RETURN_IF_ERROR(append_quoted(out, s)); }
    else { RETURN_IF_ERROR(string_buffer_append(out, "\"\"")); }
    RETURN_IF_ERROR(string_buffer_append(out, "]"));
...
static bool slp_read_u8(const u8* base, size_t size, size_t* pos, u8* out) {
    if (*pos + 1 > size) return false; *out = base[*pos]; (*pos)++; return true;
}
static bool slp_read_u16(const u8* base, size_t size, size_t* pos, u16* out) {
    if (*pos + 2 > size) return false; *out = (u16)(base[*pos] | (base[*pos+1] << 8)); (*pos)+=2; return true;
}
static bool slp_read_u32(const u8* base, size_t size, size_t* pos, u32* out) {
    if (*pos + 4 > size) return false; *out = (u32)(base[*pos] | (base[*pos+1] << 8) | (base[*pos+2] << 16) | (base[*pos+3] << 24)); (*pos)+=4; return true;
}
static bool slp_read_double(const u8* base, size_t size, size_t* pos, double* out) {
    if (*pos + 8 > size) return false; 
    union { u64 u; double d; } u;
    u.u = ((u64)base[*pos+7] << 56) | ((u64)base[*pos+6] << 48) | ((u64)base[*pos+5] << 40) | ((u64)base[*pos+4] << 32)
        | ((u64)base[*pos+3] << 24) | ((u64)base[*pos+2] << 16) | ((u64)base[*pos+1] << 8) | (u64)base[*pos];
    *out = u.d; (*pos) += 8; return true;
}
...
static bool slp_parse_values(const u8* base, size_t size, u32 offset, u32 num_items, SLPValue* out_vals) {
    ...
    while (written < num_items) {
        u8 taglen;
        if (!slp_read_u8(base, size, &pos, &taglen)) return false;
        SLPTagType tag = (SLPTagType)((taglen >> 4) & 0x7);
        u32 length = (u32)(taglen & 0x0F);
        if ((taglen >> 7) & 0x1) {
            /* extended length: low nibble << 8 | next byte */
            u8 extra; if (!slp_read_u8(base, size, &pos, &extra)) return false;
        ...
        for (u32 i = 0; i < length; i++) {
            ...
            switch (tag) {
                ...
                case SLP_NumberTag: if (!slp_read_double(base, size, &pos, &tmp.v.num)) return false; break;
                case SLP_LongStringTag: if (!slp_read_u32(base, size, &pos, &tmp.v.str_id)) return false; break;
                case SLP_ShortStringTag: {
                    u16 sid; if (!slp_read_u16(base, size, &pos, &sid)) return false; tmp.v.str_id = sid; break;
                }
                case SLP_ByteStringTag: {
                    u8 sid; if (!slp_read_u8(base, size, &pos, &sid)) return false; tmp.v.str_id = sid; break;
                }
                case SLP_IntegerTag: if (!slp_read_u32(base, size, &pos, &tmp.v.intv)) return false; break;
...
static Result slp_append_value_js(HBCReader* r, const SLPValue* v, bool for_key, StringBuffer* out) {
    switch (v->tag) {
        ...
        case SLP_IntegerTag: {
            char nb[32]; snprintf(nb, sizeof(nb), "%u", (unsigned)v->v.intv); return string_buffer_append(out, nb);
        }
        case SLP_NumberTag: {
            char db[64];
        ...
        case SLP_ByteStringTag: {
            ...
            if (s) return append_quoted(out, s);
            return string_buffer_append(out, "\"\"");
        }
        default:
...
Result format_object_literal(HBCReader* r, u32 key_count, u32 value_count, u32 keys_id, u32 values_id, StringBuffer* out, bool pretty) {
    ...
    if (pol == LITERALS_PRETTY_ALWAYS) pretty = true; else if (pol == LITERALS_PRETTY_NEVER) pretty = false;
    ...
    if (!key_vals) return ERROR_RESULT(RESULT_ERROR_MEMORY_ALLOCATION, "oom key slp");
    SLPValue* val_vals = (SLPValue*)calloc(n, sizeof(SLPValue));
    if (!val_vals) { free(key_vals); return ERROR_RESULT(RESULT_ERROR_MEMORY_ALLOCATION, "oom val slp"); }
    ...
    for (u32 i = 0; i < n; i++) {
        if (multiline) RETURN_IF_ERROR(string_buffer_append(out, "  "));
        if (!multiline && i) RETURN_IF_ERROR(string_buffer_append(out, ", "));
        ...
        if (multiline) RETURN_IF_ERROR(string_buffer_append(out, i + 1 < n ? ",\n" : "\n"));
    }
...
Result format_array_literal(HBCReader* r, u32 value_count, u32 array_id, StringBuffer* out, bool pretty) {
    ...
    if (!vals) return ERROR_RESULT(RESULT_ERROR_MEMORY_ALLOCATION, "oom arr slp");
    ...
    for (u32 i = 0; i < value_count; i++) {
        ...
        if (multiline) RETURN_IF_ERROR(string_buffer_append(out, i + 1 < value_count ? ",\n" : "\n"));
    }
    free(vals);
    RETURN_IF_ERROR(string_buffer_append(out, "]"));
    return SUCCESS_RESULT();
...
Result format_variadic_call(const ParsedInstruction* insn, StringBuffer* out) {
    ...
    if (argc > 0) {
        ...
        for (u32 i = 1; i <= argc; i++) {
            RETURN_IF_ERROR(string_buffer_append(out, ", "));
            char nb[16]; snprintf(nb, sizeof(nb), "r%u", base + i);
            RETURN_IF_ERROR(string_buffer_append(out, nb));
        }
        /* Also annotate argc */
        char cm[32]; snprintf(cm, sizeof(cm), " /*argc:%u*/", argc);
