...
Result token_string_init(TokenString* ts, ParsedInstruction* insn) {
    if (!ts) return ERROR_RESULT(RESULT_ERROR_INVALID_ARGUMENT, "token_string_init: ts NULL");
...
Result token_string_add_token(TokenString* ts, Token* t) {
    if (!ts || !t) return ERROR_RESULT(RESULT_ERROR_INVALID_ARGUMENT, "token_string_add_token: NULL");
    if (!ts->head) {
        ts->head = ts->tail = t;
    } else {
        ts->tail->next = t;
...
Token* create_raw_token(const char* text) {
    ...
    if (!rt->text) { free(rt); return NULL; }
...
Token* create_left_hand_reg_token(int reg_num) {
    ...
    if (!t) return NULL; t->reg_num = reg_num; return (Token*)t;
}
...
Token* create_left_parenthesis_token(void) { return alloc_token(TOKEN_TYPE_LEFT_PARENTHESIS, 0); }
Token* create_right_parenthesis_token(void) { return alloc_token(TOKEN_TYPE_RIGHT_PARENTHESIS, 0); }
Token* create_dot_accessor_token(void) { return alloc_token(TOKEN_TYPE_DOT_ACCESSOR, 0); }
...
Token* create_return_directive_token(void) { return alloc_token(TOKEN_TYPE_RETURN_DIRECTIVE, 0); }
Token* create_throw_directive_token(void) { return alloc_token(TOKEN_TYPE_THROW_DIRECTIVE, 0); }
...
Token* create_function_table_index_token(u32 function_id, struct HermesDecompiler* state) {
    ...
    if (!t) return NULL; t->function_id = function_id; t->state = state; t->environment_id = 0; t->is_closure = false; t->is_builtin = false; t->is_generator = false; t->is_async = false; t->parent_environment = NULL; return (Token*)t;
}
...
Token* create_jump_condition_token(u32 target_address) {
    ...
    if (!t) return NULL; t->target_address = target_address; return (Token*)t;
}
...
Token* create_get_environment_token(int reg_num, int nesting_level) {
    ...
    if (!t) return NULL; t->reg_num = reg_num; t->nesting_level = nesting_level; return (Token*)t;
}
...
Token* create_load_from_environment_token(int reg_num, int slot_index) {
    ...
    if (!t) return NULL; t->reg_num = reg_num; t->slot_index = slot_index; return (Token*)t;
}
...
Token* create_new_inner_environment_token(int dest_register, int parent_register, int number_of_slots) {
    ...
    if (!t) return NULL; t->dest_register = dest_register; t->parent_register = parent_register; t->number_of_slots = number_of_slots; return (Token*)t;
}
...
Token* create_store_to_environment_token(int env_register, int slot_index, int value_register) {
    ...
    if (!t) return NULL; t->env_register = env_register; t->slot_index = slot_index; t->value_register = value_register; return (Token*)t;
}
...
Token* create_for_in_loop_init_token(int obj_props_register, int obj_register, int iter_index_register, int iter_size_register) {
    ...
    if (!t) return NULL; t->obj_props_register = obj_props_register; t->obj_register = obj_register; t->iter_index_register = iter_index_register; t->iter_size_register = iter_size_register; return (Token*)t;
}
...
Token* create_for_in_loop_next_iter_token(int next_value_register, int obj_props_register, int obj_register, int iter_index_register, int iter_size_register) {
    ...
    if (!t) return NULL; t->next_value_register = next_value_register; t->obj_props_register = obj_props_register; t->obj_register = obj_register; t->iter_index_register = iter_index_register; t->iter_size_register = iter_size_register; return (Token*)t;
}
...
Token* create_resume_generator_token(int result_out_reg, int return_bool_out_reg) {
    ...
    if (!t) return NULL; t->result_out_reg = result_out_reg; t->return_bool_out_reg = return_bool_out_reg; return (Token*)t;
}
...
Token* create_save_generator_token(u32 address) {
    ...
    if (!t) return NULL; t->address = address; return (Token*)t;
}

Token* create_start_generator_token(void) { return alloc_token(TOKEN_TYPE_START_GENERATOR, 0); }
...
Token* create_catch_block_start_token(int arg_register) {
    ...
    if (!t) return NULL; t->arg_register = arg_register; return (Token*)t;
}
...
static Result append_reg(StringBuffer* b, int r) {
    char buf[32]; snprintf(buf, sizeof(buf), "r%d", r);
...
Result token_to_string(Token* token, StringBuffer* buffer) {
    if (!token || !buffer) return ERROR_RESULT(RESULT_ERROR_INVALID_ARGUMENT, "token_to_string: NULL");
    switch (token->type) {
        case TOKEN_TYPE_RAW: {
            RawToken* t = (RawToken*)token; return string_buffer_append(buffer, t->text ? t->text : "");
        }
        case TOKEN_TYPE_LEFT_HAND_REG: {
            LeftHandRegToken* t = (LeftHandRegToken*)token; return append_reg(buffer, t->reg_num);
        }
        case TOKEN_TYPE_RIGHT_HAND_REG: {
            RightHandRegToken* t = (RightHandRegToken*)token; return append_reg(buffer, t->reg_num);
        }
        ...
        case TOKEN_TYPE_BIND: {
            BindToken* t = (BindToken*)token; RETURN_IF_ERROR(string_buffer_append(buffer, ".bind(")); RETURN_IF_ERROR(append_reg(buffer, t->reg_num)); return string_buffer_append(buffer, ")");
        }
        ...
        case TOKEN_TYPE_FUNCTION_TABLE_INDEX: {
            ...
            char buf[32]; snprintf(buf, sizeof(buf), "fn_%u", t->function_id);
            return string_buffer_append(buffer, buf);
        }
        case TOKEN_TYPE_JUMP_CONDITION: {
            JumpConditionToken* t = (JumpConditionToken*)token; char buf[64]; snprintf(buf, sizeof(buf), "/* jump -> 0x%08x */", t->target_address); return string_buffer_append(buffer, buf);
        }
        case TOKEN_TYPE_JUMP_NOT_CONDITION: {
            JumpNotConditionToken* t = (JumpNotConditionToken*)token; char buf[64]; snprintf(buf, sizeof(buf), "/* jump_if_not -> 0x%08x */", t->target_address); return string_buffer_append(buffer, buf);
        }
        case TOKEN_TYPE_GET_ENVIRONMENT: {
            GetEnvironmentToken* t = (GetEnvironmentToken*)token; RETURN_IF_ERROR(string_buffer_append(buffer, "get_env(")); RETURN_IF_ERROR(append_reg(buffer, t->reg_num)); RETURN_IF_ERROR(string_buffer_append(buffer, ", ")); char nb[16]; snprintf(nb, sizeof(nb), "%d", t->nesting_level); RETURN_IF_ERROR(string_buffer_append(buffer, nb)); return string_buffer_append(buffer, ")");
        }
        case TOKEN_TYPE_LOAD_FROM_ENVIRONMENT: {
            LoadFromEnvironmentToken* t = (LoadFromEnvironmentToken*)token; RETURN_IF_ERROR(string_buffer_append(buffer, "env_load(")); RETURN_IF_ERROR(append_reg(buffer, t->reg_num)); RETURN_IF_ERROR(string_buffer_append(buffer, ", ")); char nb[16]; snprintf(nb, sizeof(nb), "%d", t->slot_index); RETURN_IF_ERROR(string_buffer_append(buffer, nb)); return string_buffer_append(buffer, ")");
        }
        case TOKEN_TYPE_NEW_ENVIRONMENT: {
            NewEnvironmentToken* t = (NewEnvironmentToken*)token; RETURN_IF_ERROR(string_buffer_append(buffer, "new_env(")); RETURN_IF_ERROR(append_reg(buffer, t->reg_num)); return string_buffer_append(buffer, ")");
        }
        case TOKEN_TYPE_NEW_INNER_ENVIRONMENT: {
            NewInnerEnvironmentToken* t = (NewInnerEnvironmentToken*)token; char nb[64]; snprintf(nb, sizeof(nb), "new_inner_env(r%d, r%d, %d)", t->dest_register, t->parent_register, t->number_of_slots); return string_buffer_append(buffer, nb);
        }
        case TOKEN_TYPE_SWITCH_IMM: {
            SwitchImmToken* t = (SwitchImmToken*)token; char nb[128]; snprintf(nb, sizeof(nb), "switch(r%d /*%u..%u*/)", t->value_reg, t->unsigned_min_value, t->unsigned_max_value); return string_buffer_append(buffer, nb);
        }
        case TOKEN_TYPE_STORE_TO_ENVIRONMENT: {
            StoreToEnvironmentToken* t = (StoreToEnvironmentToken*)token; char nb[64]; snprintf(nb, sizeof(nb), "env_store(r%d, %d, r%d)", t->env_register, t->slot_index, t->value_register); return string_buffer_append(buffer, nb);
        }
        case TOKEN_TYPE_FOR_IN_LOOP_INIT: {
            ForInLoopInitToken* t = (ForInLoopInitToken*)token; char nb[128]; snprintf(nb, sizeof(nb), "forin_init(r%d, r%d, r%d, r%d)", t->obj_props_register, t->obj_register, t->iter_index_register, t->iter_size_register); return string_buffer_append(buffer, nb);
        }
        case TOKEN_TYPE_FOR_IN_LOOP_NEXT_ITER: {
            ForInLoopNextIterToken* t = (ForInLoopNextIterToken*)token; char nb[160]; snprintf(nb, sizeof(nb), "forin_next(r%d, r%d, r%d, r%d, r%d)", t->next_value_register, t->obj_props_register, t->obj_register, t->iter_index_register, t->iter_size_register); return string_buffer_append(buffer, nb);
        }
        case TOKEN_TYPE_RESUME_GENERATOR: {
            ResumeGeneratorToken* t = (ResumeGeneratorToken*)token; char nb[64]; snprintf(nb, sizeof(nb), "resume_gen(r%d, r%d)", t->result_out_reg, t->return_bool_out_reg); return string_buffer_append(buffer, nb);
        }
        case TOKEN_TYPE_SAVE_GENERATOR: {
            SaveGeneratorToken* t = (SaveGeneratorToken*)token; char nb[64]; snprintf(nb, sizeof(nb), "save_gen(0x%08x)", t->address); return string_buffer_append(buffer, nb);
        }
        case TOKEN_TYPE_START_GENERATOR: return string_buffer_append(buffer, "start_generator()");
        case TOKEN_TYPE_CATCH_BLOCK_START: {
            CatchBlockStartToken* t = (CatchBlockStartToken*)token; char nb[32]; snprintf(nb, sizeof(nb), "catch(r%d)", t->arg_register); return string_buffer_append(buffer, nb);
        }
        default:
...
Result token_string_to_string(TokenString* ts, StringBuffer* out) {
    if (!ts || !out) return ERROR_RESULT(RESULT_ERROR_INVALID_ARGUMENT, "token_string_to_string: NULL");
    Token* cur = ts->head;
    ...
    while (cur) {
        ...
        if (cur != ts->head) {
            ...
            if (is_punct(cur->type) || prev == TOKEN_TYPE_LEFT_PARENTHESIS || cur->type == TOKEN_TYPE_RIGHT_PARENTHESIS || cur->type == TOKEN_TYPE_DOT_ACCESSOR)
                need_space = false;
            if (prev == TOKEN_TYPE_DOT_ACCESSOR) need_space = false;
            if (prev == TOKEN_TYPE_ASSIGNMENT || cur->type == TOKEN_TYPE_ASSIGNMENT) need_space = true;
            if (need_space) RETURN_IF_ERROR(string_buffer_append_char(out, ' '));
        }
