...
Result _hbc_disassembler_init(Disassembler *disassembler, HBCReader *reader, HBCDisOptions options) {
	...
	return _hbc_string_buffer_init (&disassembler->output, 8192);
}

/* Clean up disassembler */
void _hbc_disassembler_cleanup(Disassembler *disassembler) {
	if (disassembler) {
		_hbc_string_buffer_free (&disassembler->output);
	}
}
...
static Result print_instruction_asm(Disassembler *disassembler, ParsedInstruction *instruction) {
	...
	for (int i = 0; i < 6; i++) {
		...
		RETURN_IF_ERROR (_hbc_string_buffer_append (out, first? " ": ", "));
		first = false;
...
Result _hbc_print_instruction(Disassembler *disassembler, ParsedInstruction *instruction) {
	...
	for (int i = 0; i < 6; i++) {
		...
		RETURN_IF_ERROR (_hbc_string_buffer_append (output, operand_name));
		RETURN_IF_ERROR (_hbc_string_buffer_append (output, ": "));
	...
	for (int i = 0; i < 6; i++) {
		...
		case OPERAND_MEANING_STRING_ID:
			if (value < reader->header.stringCount &&
				...
				RETURN_IF_ERROR (_hbc_string_buffer_append (output, "  # String: \""));
				RETURN_IF_ERROR (_hbc_string_buffer_append (output, reader->strings[value]));
				RETURN_IF_ERROR (_hbc_string_buffer_append (output, "\" ("));
				if (value < reader->header.stringCount && reader->string_kinds) {
					RETURN_IF_ERROR (_hbc_string_buffer_append (output, _hbc_string_kind_to_string (reader->string_kinds[value])));
				} else {
					RETURN_IF_ERROR (_hbc_string_buffer_append (output, "Unknown"));
				}
				RETURN_IF_ERROR (_hbc_string_buffer_append (output, ")"));
			}
			break;
		...
		case OPERAND_MEANING_BIGINT_ID:
			if (value < reader->bigint_count) {
				RETURN_IF_ERROR (_hbc_string_buffer_append (output, "  # BigInt: "));
				RETURN_IF_ERROR (_hbc_string_buffer_append_int (output, reader->bigint_values[value]));
			}
			break;
	...
	if (strcmp (instruction->inst->name, "SwitchImm") == 0) {
		if (instruction->switch_jump_table && instruction->switch_jump_table_size > 0) {
			RETURN_IF_ERROR (_hbc_string_buffer_append (output, "  # Jump table: ["));
...
Result _hbc_disassemble_function(Disassembler *disassembler, u32 function_id) {
	...
	if (disassembler->options.asm_syntax) {
		RETURN_IF_ERROR (_hbc_string_buffer_append (&disassembler->output, "Bytecode listing (asm):\n\n"));
	} else {
		RETURN_IF_ERROR (_hbc_string_buffer_append (&disassembler->output, "Bytecode listing:\n\n"));
	}
	...
	if (function_header->bytecodeSizeInBytes > 0 && !function_header->bytecode) {
		...
		if (!function_header->bytecode) {
			RETURN_IF_ERROR (_hbc_string_buffer_append (&disassembler->output, "[Memory allocation failed for bytecode]\n"));
		...
		if (seek_result.code != RESULT_SUCCESS) {
			RETURN_IF_ERROR (_hbc_string_buffer_append (&disassembler->output, "[Failed to seek to bytecode location]\n"));
		...
		if (read_result.code != RESULT_SUCCESS) {
			RETURN_IF_ERROR (_hbc_string_buffer_append (&disassembler->output, "[Failed to read bytecode data]\n"));
			free (function_header->bytecode);
			function_header->bytecode = NULL;
			return read_result;
		}
