...
#define E3(mid) \
	{ \
		r_strbuf_setf (&op->esil, "r%u,r%u," mid ",r%u,=", bytes[3], bytes[2], bytes[1]); \
		break; \
	}
#define E2(mid) \
	{ \
		r_strbuf_setf (&op->esil, "r%u," mid ",r%u,=", bytes[2], bytes[1]); \
		break; \
	}
#define EDST(pre) \
	{ \
		r_strbuf_setf (&op->esil, pre ",r%u,=", bytes[1]); \
		break; \
	}
#define JMP_S \
	{ \
		r_strbuf_setf (&op->esil, "0x%" PFMT64x ",pc,=", addr + (i8)bytes[1]); \
		break; \
	}
#define JMP_L \
	{ \
		r_strbuf_setf (&op->esil, "0x%" PFMT64x ",pc,=", addr + (i32)r_read_le32 (bytes + 1)); \
		break; \
	}
#define JC1_S(pre) \
	{ \
		r_strbuf_setf (&op->esil, "r%u," pre "?{,0x%" PFMT64x ",pc,=,}", bytes[2], addr + (i8)bytes[1]); \
		break; \
	}
#define JC1_L(pre) \
	{ \
		r_strbuf_setf (&op->esil, "r%u," pre "?{,0x%" PFMT64x ",pc,=,}", bytes[5], addr + (i32)r_read_le32 (bytes + 1)); \
		break; \
	}
#define JCC_S(cmp) \
	{ \
		r_strbuf_setf (&op->esil, "r%u,r%u," cmp ",?{,0x%" PFMT64x ",pc,=,}", bytes[3], bytes[2], addr + (i8)bytes[1]); \
		break; \
	}
#define JCC_L(cmp) \
	{ \
		r_strbuf_setf (&op->esil, "r%u,r%u," cmp ",?{,0x%" PFMT64x ",pc,=,}", bytes[6], bytes[5], addr + (i32)r_read_le32 (bytes + 1)); \
		break; \
	}
	...
	case OP_GetById:
	case OP_GetByIdLong:
	case OP_GetByIdShort: E2 ("GETPROP")
	case OP_TryGetById:
	case OP_TryGetByIdLong: E2 ("TRYGETPROP")
	case OP_DelById:
	case OP_DelByIdLong: E2 ("DELPROP")
	...
	case OP_Construct:
	case OP_ConstructLong: E2 ("CONSTRUCT")
	case OP_Call:
	case OP_CallLong: E2 ("CALL")
	...
	case OP_CreateClosureLongIndex: EDST ("CLOSURE")
	case OP_CreateAsyncClosure:
	case OP_CreateAsyncClosureLongIndex: EDST ("ASYNCCLOSURE")
	case OP_CreateGeneratorClosure:
	case OP_CreateGeneratorClosureLongIndex: EDST ("GENCLOSURE")
	case OP_CallDirect:
	case OP_CallDirectLongIndex: EDST ("CALLDIRECT")
	case OP_Catch: EDST ("CATCH")
	case OP_NewArrayWithBuffer:
	case OP_NewArrayWithBufferLong: EDST ("NEWARRBUF")
	case OP_NewObjectWithBuffer:
	case OP_NewObjectWithBufferLong: EDST ("NEWOBJBUF")
	case OP_GetArgumentsPropByVal:
	case OP_GetArgumentsLength:
	case OP_ReifyArguments: EDST ("arguments")
	case OP_CreateRegExp: EDST ("REGEXP")
	case OP_CallBuiltin:
	case OP_CallBuiltinLong: EDST ("BUILTIN")
	...
	case OP_JLess:
	case OP_JLessN: JCC_S ("<")
	case OP_JLessLong:
	case OP_JLessNLong: JCC_L ("<")
	case OP_JNotLess:
	case OP_JNotLessN: JCC_S (">=")
	case OP_JNotLessLong:
	case OP_JNotLessNLong: JCC_L (">=")
	case OP_JLessEqual:
	case OP_JLessEqualN: JCC_S ("<=")
	case OP_JLessEqualLong:
	case OP_JLessEqualNLong: JCC_L ("<=")
	case OP_JGreater:
	case OP_JGreaterN: JCC_S (">")
	case OP_JGreaterLong:
	case OP_JGreaterNLong: JCC_L (">")
	case OP_JGreaterEqual:
	case OP_JGreaterEqualN: JCC_S (">=")
	case OP_JGreaterEqualLong:
	case OP_JGreaterEqualNLong: JCC_L (">=")
	case OP_JNotLessEqual:
	case OP_JNotLessEqualN: JCC_S (">")
	case OP_JNotLessEqualLong:
	case OP_JNotLessEqualNLong: JCC_L (">")
	case OP_JNotGreater:
	case OP_JNotGreaterN: JCC_S ("<=")
	case OP_JNotGreaterLong:
	case OP_JNotGreaterNLong: JCC_L ("<=")
